        ...
        private getCompileOnSaveAffectedFileList(args: protocol.FileRequestArgs): ReadonlyArray<protocol.CompileOnSaveAffectedFileListSingleProject> {
            ...
            return combineProjectOutput(
                ...
                (project, info) => {
                    ...
                    if (!!compilationSettings.noEmit || fileExtensionIs(info.fileName, Extension.Dts) && (!!compilationSettings.skipDTSCompileOnSave || !getEmitDeclarations(compilationSettings))) {
                        // avoid triggering emit when a change is made in a .d.ts when either declaration emit is disabled or when specifically requested
                        return undefined;
                    }
